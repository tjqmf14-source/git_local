...
- 충돌 없는 분기 상태는 자동 병합하고, 충돌 발생 시 merge를 자동 취소해 시작 전 상태로 복구
...
- behind-only는 fast-forward만 사용하고, diverged는 일반 merge를 시도하되 충돌 시 즉시 abort하여 원래 HEAD/작업트리로 복구합니다.
- 다른 origin을 발견하면 자동 덮어쓰기하지 않습니다.
- 인증정보가 포함된 GitHub URL은 등록하지 않습니다.

## QA

`tests/Run-Tests.ps1`이 임시 bare Git 저장소를 직접 생성하여 clone, commit/push, fast-forward, ahead-only, diverged 자동 병합, 충돌 rollback, dirty 보호, 한글 경로, 예외 처리를 검증합니다. Windows Actions는 UI self-test, EXE self-test, ZIP 및 SHA-256 생성까지 확인합니다.
