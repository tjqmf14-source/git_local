...
function Assert-ThrowsMatch([scriptblock]$action,[string]$pattern,[string]$name) {
    ...
    if ($message -notmatch $pattern) { Fail $name ("예상 오류 패턴='$pattern' actual='$message'") }
    Pass $name
}
...
$forcedExitLines = @(Get-Content -LiteralPath $PSCommandPath | Where-Object { $_ -match '^\s*exit\s+0\s*(#.*)?
...
$lastExitOverwriteLines = @(Get-Content -LiteralPath $PSCommandPath | Where-Object { $_ -match '^\s*\$global:LASTEXITCODE\s*=\s*0\s*
$base = Join-Path ([System.IO.Path]::GetTempPath()) ('GitLocal-QA-' + [Guid]::NewGuid().ToString('N'))
$configRoot = Join-Path $base 'config'
$remote = Join-Path $base 'remote.git'
$seed = Join-Path $base 'seed'
...
$hiddenFailureLines = @($workflowLines | Where-Object { $_ -match '^\s*continue-on-error\s*:\s*true\s*
$base = Join-Path ([System.IO.Path]::GetTempPath()) ('GitLocal-QA-' + [Guid]::NewGuid().ToString('N'))
$configRoot = Join-Path $base 'config'
$remote = Join-Path $base 'remote.git'
$seed = Join-Path $base 'seed'
